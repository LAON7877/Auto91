...
def order_callback(state, deal, order=None):
    ...
    try:
        ...
        if octype_info is None:
            ...
            # ⚠️ 應急推斷機制（只做臨時使用，不保存到映射中）
            # 這個分支現在應該很少發生，因為我們已經在API調用前預先保存了
            try:
                action = deal.get('order', {}).get('action', deal.get('action', 'Sell'))
...
def handle_futures_deal_callback(deal, octype_info):
    ...
    try:
        ...
        # 🎯 關鍵修復：完全使用octype_info中的原始數據，絕不重新推斷！
        order_type = octype_info.get('order_type', 'IOC')  # 從映射中獲取原始訂單類型
        price_type = octype_info.get('price_type', 'MKT')  # 從映射中獲取原始價格類型
        is_manual = octype_info.get('is_manual', True)     # 從映射中獲取原始來源判斷
        
        # ❌ 刪除所有重新推斷邏輯，完全信任octype_info中的數據
        logger.info(f"🎯 使用原始訂單參數: order_type={order_type}, price_type={price_type}, is_manual={is_manual}")
        
        # ✅ 這樣確保成交通知與提交通知使用完全相同的數據
