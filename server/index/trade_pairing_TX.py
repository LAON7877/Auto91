...
def record_opening_trade(contract_code, action, quantity, price, order_id):
    ...
    try:
        ...
        open_file = os.path.join(TRADE_RECORDS_DIR, f'tx_open_positions_{today}.json')
...
def record_covering_trade(contract_code, action, quantity, price, order_id):
    ...
    try:
        ...
        for i in range(90):  # 查找過去90天的開倉記錄
            check_date = datetime.now() - timedelta(days=i)
            date_str = check_date.strftime('%Y%m%d')
            open_file = os.path.join(TRADE_RECORDS_DIR, f'tx_open_positions_{date_str}.json')
        ...
        today = get_today_str()
        os.makedirs(TRADE_RECORDS_DIR, exist_ok=True)  # 只在有數據時創建
        cover_file = os.path.join(TRADE_RECORDS_DIR, f'tx_cover_trades_{today}.json')
...
def get_trading_statistics(date_range_days=1):
    ...
    try:
        ...
        for i in range(date_range_days):
            ...
            # 統計平倉
            cover_file = os.path.join(TRADE_RECORDS_DIR, f'tx_cover_trades_{date_str}.json')
...
def get_tx_cover_trades_for_report(date_range_days=1):
    """獲取平倉交易明細供報表使用
...
def cleanup_old_tx_files():
    ...
    try:
        ...
        # 清理TX交易記錄目錄
        for file_pattern in ['tx_open_positions_*.json', 'tx_cover_trades_*.json']:
        ...
        logger.info(f"✅ TX檔案清理完成，保留最近90天數據")
