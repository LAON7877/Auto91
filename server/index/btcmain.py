...
def initialize_btc_directories():
    ...
    try:
        ...
        # 執行BTC數據文件清理（10%機率，參考TX系統）
        import random
        if random.random() < 0.1:  # 10%機率執行清理
            logger.info("🧹 執行BTC數據文件清理...")
            cleanup_old_btc_data_files()
...
def calculate_btc_add_position_quantity(signal_data, account_balance, action):
    """計算BTC加倉數量（同方向持倉時使用）"""
    try:
        # 獲取配置的槓桿倍數
        env_data = load_btc_env_data()
        ...
        # 🔧 加倉邏輯：使用較小的倉位比例避免保證金不足
        # 獲取原倉位大小百分比並減半
        original_position_size_pct = float(env_data.get('POSITION_SIZE', 10.0)) / 100
        add_position_size_pct = original_position_size_pct * 0.3  # 加倉使用30%的原倉位比例
        
        # 計算可用餘額
        available_balance = float(account_balance.get('availableBalance', 0))
        
        # 檢查可用餘額是否足夠
        if available_balance < 10:  # 至少需要10 USDT
            logger.warning(f"⚠️ 可用餘額不足以進行加倉: ${available_balance:.2f}")
            return 0.001  # 返回最小單位
        
        # 保證金 = 可用餘額 * 加倉倉位比例
        margin_used = available_balance * add_position_size_pct
        # 下單金額 = 保證金 * 槓桿倍數
        order_value = margin_used * leverage
        
        logger.info(f"📊 加倉計算: 原倉位比例{original_position_size_pct*100}% -> 加倉比例{add_position_size_pct*100}%")
        logger.info(f"💰 加倉計算: 可用餘額${available_balance:.2f} × {add_position_size_pct*100}%倉位 = ${margin_used:.2f}保證金")
        logger.info(f"🎯 加倉槓桿: ${margin_used:.2f}保證金 × {leverage}倍槓桿 = ${order_value:.2f}下單金額")
        ...
        logger.error(f"計算BTC加倉數量失敗: {e}")
        return 0.001  # 返回最小單位

def calculate_btc_add_position_quantity_6_percent(signal_data, account_balance, action):
    """計算BTC加倉數量（固定6%比例）"""
    try:
        # 獲取配置的槓桿倍數
        env_data = load_btc_env_data()
        ...
        # 檢查可用餘額是否足夠
        if available_balance < 10:  # 至少需要10 USDT
            logger.warning(f"⚠️ 可用餘額不足以進行6%加倉: ${available_balance:.2f}")
            return 0.001  # 返回最小單位
        ...
        logger.info(f"📊 6%加倉計算: 可用餘額${available_balance:.2f} × 6% = ${margin_used:.2f}保證金")
        logger.info(f"🎯 6%加倉槓桿: ${margin_used:.2f}保證金 × {leverage}倍槓桿 = ${order_value:.2f}下單金額")
        ...
        logger.error(f"計算BTC 6%加倉數量失敗: {e}")
        return 0.001  # 返回最小單位

def place_btc_futures_order(symbol, side, quantity, price=None, order_type="MARKET", reduce_only=False, is_manual=False, suppress_submit_notification=False, pre_close_data=None):
    """BTC期貨下單 - 支援完整通知功能"""
    ...
    try:
        ...
        # 🔧 統一使用 get_btc_action_and_direction 函數確保一致性
        trade_record = {
            'side': side,
            'reduceOnly': reduce_only,
        ...
        parsed_action, direction = get_btc_action_and_direction(trade_record)
        ...
        # 只記錄後端日誌，文件記錄由Websocket成交回調統一處理
        ...
        if result:
            ...
            trade_record = {
                ...
                'order_id': safe_order_id(order_id),  # 🔧 金融級數據類型安全
                'order_type': order_type_text,
            ...
            # 🔧 如果是平倉且有平倉前數據，添加到trade_record
            if reduce_only and pre_close_data:
                trade_record.update({
                    'pre_close_data': pre_close_data,
                    'closing_position_amt': abs(pre_close_data['position_amt']),
                    'closing_position_value': pre_close_data['position_value'],
                    'entry_price': pre_close_data['entry_price'],
                    'unrealized_pnl': pre_close_data['unrealized_pnl'],
                    'liquidation_price': pre_close_data['liquidation_price']
                })
            ...
            if not suppress_submit_notification:
                # 🔧 使用基於訂單ID的延遲，確保同訂單通知成組發送
                import hashlib
                order_hash = int(hashlib.md5(safe_hash_string(order_id).encode()).hexdigest()[:8], 16)
                base_delay = (order_hash % 100) * 0.1  # 0-10秒的基礎延遲
                submit_delay = base_delay + 1  # 提交通知：基礎延遲 + 1秒
                
                logger.info(f"準備將BTC提交成功通知加入排隊: {trade_record}")
                btc_notification_manager.add_notification('submit', trade_record, True, submit_delay)
                logger.info(f"BTC提交成功通知已加入排隊系統（延遲{submit_delay:.1f}秒）")
            else:
        ...
        # 🔧 統一使用 get_btc_action_and_direction 函數確保一致性（用於失敗通知）
        trade_record_for_error = {
            'side': side,
            'reduceOnly': reduce_only,
            'reduce_only': reduce_only,
            'action_type': ''  # 由函數自動判斷
        }
        parsed_action, direction = get_btc_action_and_direction(trade_record_for_error)
        action = 'cover' if reduce_only else 'new'
        # 移除手動/自動標示
        order_source = ''
        formatted_quantity = f"{float(quantity):.8f}"
        ...
        # 🔧 失敗通知也使用訂單配對邏輯，但延遲較長以便排在成功通知後
        import hashlib
        order_hash = int(hashlib.md5(str(error_record.get('order_id', 'error')).encode()).hexdigest()[:8], 16)
        base_delay = (order_hash % 100) * 0.1
        fail_delay = base_delay + 5  # 失敗通知：基礎延遲 + 5秒
        
        logger.info(f"準備將BTC提交失敗通知加入排隊: {error_record}")
        btc_notification_manager.add_notification('submit', error_record, False, fail_delay)
        logger.info(f"BTC提交失敗通知已加入排隊系統（延遲{fail_delay:.1f}秒）")
    ...
    try:
        ...
        if order_status:
            ...
            if status == 'FILLED':
                ...
                import hashlib
                order_hash = int(hashlib.md5(safe_hash_string(order_id).encode()).hexdigest()[:8], 16)
                base_delay = (order_hash % 100) * 0.1  # 與提交通知使用相同的基礎延遲
                fill_delay = base_delay + 3  # 成交通知：基礎延遲 + 3秒
                
                logger.info(f"準備將BTC成交通知加入排隊: 訂單{order_id}")
                btc_notification_manager.add_notification('fill', trade_record, True, fill_delay)
                logger.info(f"BTC成交通知已加入排隊系統（延遲{fill_delay:.1f}秒）")
                ...
                try:
                    ...
                    order_hash = int(hashlib.md5(safe_hash_string(order_id).encode()).hexdigest()[:8], 16)
                    base_delay = (order_hash % 100) * 0.1
                    fail_delay = base_delay + 5  # 失效通知：基礎延遲 + 5秒
...
def auto_close_opposite_position(symbol, target_action):
    ...
    try:
        if not binance_client:
            logger.error("❌ BTC客戶端未初始化，無法執行平倉")
            return {'error': 'BTC客戶端未初始化'}
            
        logger.info(f"🔍 開始查找{symbol}的活躍持倉進行平倉")
        
        # 🔧 修復：使用正確的幣安期貨API方法
        positions = binance_client.futures_position_information(symbol=symbol)
        if not positions:
            logger.warning("⚠️ 無法獲取持倉信息，API返回空值")
            return {'error': '無法獲取持倉信息'}
            
        logger.info(f"📊 API響應: 獲取到{len(positions)}個持倉記錄")
        ...
        for pos in positions:
            if pos.get('symbol') == symbol:
                ...
                logger.info(f"📈 {symbol}持倉: {position_amt:.8f}")
        ...
        if not target_position:
            logger.info(f"📊 沒有找到{symbol}的活躍持倉(>0.001)，跳過平倉")
        ...
        if (target_action == 'LONG' and current_side == 'SHORT') or \
           (target_action == 'SHORT' and current_side == 'LONG'):
            ...
            try:
                ...
                liquidation_price = float(target_position.get('liquidationPrice', 0))
                position_value = abs(position_amt * entry_price) if entry_price > 0 else 0
                ...
                logger.info(f"✅ 對翻平倉前數據已保存: 持倉{position_amt:.8f}BTC, 價值{position_value:.2f}USDT")
            except Exception as e:
                logger.error(f"❌ 對翻平倉前獲取數據失敗: {e}")
                pre_close_data = None
            ...
            close_result = place_btc_futures_order(
                ...
                pre_close_data=pre_close_data  # 🔧 傳遞平倉前數據
            )
...
def check_opposite_position(action):
    ...
    try:
        if not binance_client:
            logger.warning("⚠️ BTC客戶端未初始化，無法檢測持倉")
            return False
            
        logger.info(f"🔍 開始檢測反向持倉: 目標動作={action}")
        ...
        for pos in positions:
            if pos.get('symbol') == 'BTCUSDT':
                ...
                logger.info(f"📈 BTCUSDT持倉: {position_amt:.8f} BTC, 開倉價格: {entry_price:.2f}")
                
                # 🔧 調整：使用合理的持倉檢測閾值（0.001 BTC）
                if abs(position_amt) > 0.001:
                    current_side = 'LONG' if position_amt > 0 else 'SHORT'
                    ...
                    if (action == 'LONG' and current_side == 'SHORT') or \
                       (action == 'SHORT' and current_side == 'LONG'):
                        logger.info(f"✅ 發現反向持倉: 當前{current_side}({abs(position_amt):.8f} BTC)，嘗試開{action}")
                        return True
                    else:
                        logger.info(f"🔄 發現同方向持倉: 當前{current_side}({abs(position_amt):.8f} BTC)，目標{action}")
                        return False
                else:
                    logger.info(f"📊 持倉數量過小({abs(position_amt):.8f} BTC)，視為無持倉")
                        
        logger.info("📊 未發現活躍持倉，可直接開倉")
        return False
        
    except Exception as e:
        logger.error(f"❌ 檢查反向持倉失敗: {e}")
        logger.error(f"   API方法: futures_position_information")
        logger.error(f"   錯誤類型: {type(e).__name__}")
        # 🔧 重要：不靜默失敗，而是重新拋出異常讓上層處理
        raise e

def check_same_direction_position(action):
    """檢查是否存在同方向持倉（用於加倉判斷）
    ...
    Returns:
        bool: True if 存在同方向持倉
    """
    try:
        if not binance_client:
            logger.warning("⚠️ BTC客戶端未初始化，無法檢測同向持倉")
            return False
            
        logger.info(f"🔍 開始檢測同方向持倉: 目標動作={action}")
        ...
        for pos in positions:
            if pos.get('symbol') == 'BTCUSDT':
                ...
                if abs(position_amt) > 0.001:
                    ...
                    # 檢查是否為同方向
                    if (action == 'LONG' and current_side == 'LONG') or \
                       (action == 'SHORT' and current_side == 'SHORT'):
                        logger.info(f"✅ 發現同方向持倉: 當前{current_side}({abs(position_amt):.8f} BTC)，嘗試加倉{action}")
                        return True
                    else:
                        logger.info(f"🔄 發現反方向持倉: 當前{current_side}({abs(position_amt):.8f} BTC)，目標{action}")
                        return False
                else:
                    logger.info(f"📊 持倉數量過小({abs(position_amt):.8f} BTC)，視為無持倉")
                        
        logger.info("📊 未發現活躍持倉，不是加倉操作")
        return False
        
    except Exception as e:
        logger.error(f"❌ 檢查同方向持倉失敗: {e}")
        logger.error(f"   API方法: futures_position_information")
        logger.error(f"   錯誤類型: {type(e).__name__}")
        # 🔧 重要：不靜默失敗，而是重新拋出異常讓上層處理
        raise e

def process_btc_unified_signal(signal_data, parsed_action=None):
    """統一的BTC交易處理函數 - 處理開倉/平倉/對翻/加倉"""
    try:
        action = parsed_action or signal_data.get('action', '').upper()
        symbol = signal_data.get('symbol', 'BTCUSDT')
        
        # 處理純平倉信號 - 此函數不應該接收到平倉訊號
        if action in ['CLOSE', 'CLOSE_LONG', 'CLOSE_SHORT']:
            logger.error(f"🚨 錯誤：process_btc_unified_signal不應該接收平倉訊號: {action}")
            logger.error(f"🚨 平倉訊號應該直接調用handle_pure_close_signal處理")
            return handle_pure_close_signal(symbol, 'CLOSE')
        
        # 處理開倉信號 (LONG/SHORT)
        if action not in ['LONG', 'SHORT']:
            raise ValueError(f"無效的交易動作: {action}")
        ...
        try:
            ...
            logger.info(f"🔍 反向持倉檢測結果: {has_opposite_position}")
        except Exception as e:
            logger.error(f"❌ 反向持倉檢測失敗，將視為無反向持倉: {e}")
            has_opposite_position = False
            
        try:
            has_same_direction_position = check_same_direction_position(action)
            logger.info(f"🔍 同向持倉檢測結果: {has_same_direction_position}")
        except Exception as e:
            logger.error(f"❌ 同向持倉檢測失敗，將視為無同向持倉: {e}")
            has_same_direction_position = False
        
        # 🔧 修復：根據您的需求實現正確的持倉處理邏輯
        if has_same_direction_position:
            # 持倉中收到同方向開倉快訊 → 直接加倉
            logger.info(f"🔍 檢測到同方向持倉，將進行加倉操作：當前持倉 + {action}")
            quantity = calculate_btc_add_position_quantity_6_percent(signal_data, usdt_balance, action)
            logger.info(f"📈 加倉數量(6%): {quantity:.8f} BTC")
        elif has_opposite_position:
            # 持倉中收到不同方向開倉快訊 → 先平倉再開倉（多空對翻）
            logger.info(f"🔍 檢測到反向持倉，將進行對翻操作：當前持倉 -> {action}")
            quantity = calculate_btc_quantity(signal_data, usdt_balance)
        else:
            # 無持倉 → 正常開倉
            logger.info(f"🔍 無持倉，將進行正常開倉操作：{action}")
            quantity = calculate_btc_quantity(signal_data, usdt_balance)
        
        if has_opposite_position:
            logger.info(f"🔍 檢測到反向持倉，開始執行對翻操作：當前持倉 -> {action}")
            ...
            # 計算平倉訂單的排隊延遲
            close_order_id = str(close_order_result.get('orderId', ''))
            close_order_hash = int(hashlib.md5(safe_hash_string(close_order_id).encode()).hexdigest()[:8], 16)
            close_base_delay = (close_order_hash % 100) * 0.1
            close_submit_delay = close_base_delay + 1
            
            btc_notification_manager.add_notification('submit', close_trade_record, True, close_submit_delay)
            logger.info(f"平倉提交成功通知已加入排隊系統（延遲{close_submit_delay:.1f}秒）")
            
            # 2. 開倉提交成功通知（使用不同的時間組，避免與平倉衝突）
            # 暫時用當前時間戳生成開倉訂單的延遲ID
            temp_open_id = f"open_{int(time.time() * 1000)}"
            open_order_hash = int(hashlib.md5(safe_hash_string(temp_open_id).encode()).hexdigest()[:8], 16)
            open_base_delay = (open_order_hash % 100) * 0.1 + 100 * 0.1  # 加10秒偏移避免衝突
            open_submit_delay = open_base_delay + 1
            ...
            open_trade_record = {
                ...
                'action_type': '開倉',
                'is_manual': False,
                'order_id': temp_open_id  # 臨時ID，實際下單後會更新
            }
            btc_notification_manager.add_notification('submit', open_trade_record, True, open_submit_delay)
            logger.info(f"開倉提交成功通知已加入排隊系統（延遲{open_submit_delay:.1f}秒）")
...
def handle_pure_close_signal(symbol, action):
    """處理純平倉信號 - 平掉所有持倉"""
    try:
        # 獲取當前持倉
        if not binance_client:
        ...
        if not target_position:
            logger.warning(f"⚠️ 平倉信號但沒有找到{symbol}的活躍持倉，直接跳過")
            return {'warning': '無持倉可平倉，跳過處理'}
        
        # 計算平倉方向和數量
        position_amt = float(target_position.get('positionAmt', 0))
        current_position_side = 'LONG' if position_amt > 0 else 'SHORT'
        
        # 平倉訊號：檢查方向匹配，只平對應方向的持倉
        if action == 'CLOSE_LONG' and current_position_side != 'LONG':
            logger.warning(f"⚠️ 收到平多訊號但當前持倉為{current_position_side}，跳過處理")
            return {'warning': '平多訊號但無多單持倉，跳過處理'}
        elif action == 'CLOSE_SHORT' and current_position_side != 'SHORT':
            logger.warning(f"⚠️ 收到平空訊號但當前持倉為{current_position_side}，跳過處理")
            return {'warning': '平空訊號但無空單持倉，跳過處理'}
        
        logger.info(f"✅ 平倉方向匹配: {action} -> {current_position_side}持倉 {abs(position_amt):.8f} BTC")
        ...
        pre_close_data = {
            'position_amt': position_amt,
            'entry_price': float(target_position.get('entryPrice', 0)),
            'unrealized_pnl': float(target_position.get('unRealizedPnl', 0)),
            'position_value': abs(position_amt) * float(target_position.get('entryPrice', 0)),
            'liquidation_price': float(target_position.get('liquidationPrice', 0)),
...
def determine_btc_log_type(message):
    """根據BTC日誌訊息內容自動判斷記錄類型（參考TX系統）"""
    if "提交成功" in message or "Telegram［提交成功］" in message:
        return "order"  # 委託提交成功
    elif "成交通知" in message or "Telegram［成交通知］" in message:
        return "deal"   # 訂單成交
    elif "提交失敗" in message or "Telegram［提交失敗］" in message:
        return "fail"   # 委託提交失敗
    elif "取消" in message or "CANCEL" in message.upper():
        return "cancel" # 訂單取消
    else:
        return "log"    # 其他系統日誌
...
def log_btc_frontend_message(message, log_type="info"):
    ...
    try:
        ...
        # 準備日誌記錄（參考TX系統格式）
        log_record = {
            'type': determine_btc_log_type(message),  # 根據訊息內容自動判斷類型
            'timestamp': datetime.now().isoformat(),
            'message': message,
            'log_type': log_type,
            'system': 'BTC'
        ...
        # 保存到今日BTC交易數據文件
        today = datetime.now().strftime("%Y%m%d")
        filename = os.path.join(btc_data_dir, f"BTCtransdata_{today}.json")
        ...
        if os.path.exists(filename):
            try:
                ...
                logger.warning(f"讀取BTC日誌文件失敗，創建新文件: {e}")
                existing_data = []
        ...
        logger.info(f"BTC日誌已保存到文件: {message}")
        return True
        
    except Exception as e:
        logger.error(f"BTC日誌文件保存失敗: {e}")
        # 降級到後端日誌
        logger.info(f"BTC日誌(降級): {message}")
        return False
...
def send_btc_order_submit_notification(trade_record, success=True):
    ...
    try:
        ...
        # 🔧 統一字段檢查：支援多種命名格式
        reduce_only = (
            trade_record.get('reduceOnly', False) or  # 駝峰格式
            trade_record.get('reduce_only', False) or  # 下劃線格式
            trade_record.get('reduceonly', False)      # 全小寫格式
        )
        
        # 🔧 智能數據獲取：平倉使用平倉前數據，開倉使用當前數據
        if reduce_only and trade_record.get('pre_close_data'):
            # 平倉：使用平倉前保存的數據
            pre_data = trade_record['pre_close_data']
            position_data = {
                'symbol': symbol,
                'contract_type': '永續',
                'margin_mode': '全倉',
                'leverage': '100X',
                'position_usdt': pre_data['position_value'],
                'entry_price': pre_data['entry_price'],
                'liquidation_price': pre_data['liquidation_price'],
                'unrealized_pnl': pre_data['unrealized_pnl']
            }
            ...
            leverage = position_data.get('leverage', '100X')
        else:
            # 開倉：使用訂單數據結合當前帳戶設定
            position_data = get_btc_position_data_for_notification(symbol)
            trading_pair = symbol  # 直接使用訂單的交易對
            contract_type = '永續'  # 幣安期貨都是永續合約
            margin_mode = position_data.get('margin_mode', '全倉')
            # 從環境配置獲取槓桿，確保一致性
            import os
            configured_leverage = os.getenv('LEVERAGE', '100')
            leverage = position_data.get('leverage', configured_leverage)
        ...
        # 🔧 判斷是否為平倉操作 - 統一邏輯
        is_close_action = (
            reduce_only or  # 標準reduce_only標記
            'cover' in action_type.lower() or  # 英文關鍵詞
            '平倉' in action_type or  # 中文關鍵詞
            '平多' in action_type or  # 具體平倉動作
            '平空' in action_type or  # 具體平倉動作
            'close' in action_type.lower() or  # 標準英文
            'exit' in action_type.lower()  # 退出關鍵詞
        )
        
        # 🔥 修復：對於平倉操作，優先使用平倉前保存的數據
        if is_close_action:
            # 優先使用平倉前保存的數據（pre_close_data）
            if reduce_only and trade_record.get('pre_close_data'):
                pre_data = trade_record['pre_close_data']
                btc_quantity = abs(float(pre_data.get('position_amt', quantity)))  # 使用平倉前的實際持倉數量
                entry_price = float(pre_data.get('entry_price', 0))
                position_usdt = btc_quantity * entry_price if entry_price > 0 else 0
                logger.info(f"🔧 使用平倉前數據 - BTC數量: {btc_quantity:.8f}, 開倉價格: {entry_price:.2f}, 持倉USDT: {position_usdt:.2f}")
            else:
                # 嘗試獲取當前實際持倉信息（僅適用於平倉提交通知瞬間）
                ...
                if actual_position_amt > 0 and actual_entry_price > 0:
                    ...
                    position_usdt = btc_quantity * entry_price if entry_price > 0 else 0
                    logger.warning(f"⚠️ 無法獲取持倉信息，使用訂單信息 - 數量: {btc_quantity:.8f}, 價格: {entry_price:.2f}")
        else:
            # 開倉操作：使用訂單信息計算預期數據
            btc_quantity = float(quantity)
            # 對於開倉，使用提交的價格計算持倉價值
            ...
            if submitted_price > 0:
                ...
                position_usdt = btc_quantity * submitted_price
            else:
                # 市價單情況，嘗試獲取當前市價
                try:
                    current_price = float(binance_client.get_mark_price(symbol='BTCUSDT').get('markPrice', 0)) if binance_client else 0
                    entry_price = current_price if current_price > 0 else position_data.get('entry_price', 0)
                    position_usdt = btc_quantity * entry_price if entry_price > 0 else 0
                except:
                    entry_price = position_data.get('entry_price', 0)
                    position_usdt = btc_quantity * entry_price if entry_price > 0 else 0
        ...
        if is_close_action:
            ...
            if position_usdt > 0:
                ...
                if submitted_price > 0:
                    ...
                    position_usdt_display = f"{btc_quantity:.8f} BTC (市價)"
        ...
        # 💰 計算盈虧（僅適用於平倉操作）
        if is_close_action and reduce_only and trade_record.get('pre_close_data'):
            pre_data = trade_record['pre_close_data']
            unrealized_pnl = pre_data.get('unrealized_pnl', 0)
            
            # 對於平倉，顯示即將實現的盈虧
            if unrealized_pnl != 0:
                pnl_sign = "+" if unrealized_pnl > 0 else ""
                pnl_display = f"\n預計實現盈虧：{pnl_sign}{unrealized_pnl:,.2f} USDT"
                logger.info(f"💰 預計實現盈虧: {unrealized_pnl:,.2f} USDT")
        ...
        if success:
            # 確保槓桿顯示格式一致
            leverage_display = f"{leverage}X" if not str(leverage).endswith('X') else leverage
            
            # 📊 調試日誌：記錄通知數據優化結果
            logger.info(f"📊 通知數據優化完成 - 動作:{action_type}, 數量:{btc_quantity:.8f} BTC, 持倉USDT:{position_usdt_display}, 價格:{entry_price_display}")
            
            msg = (f"⭕ 提交成功（{current_date}）\n"
                   f"{trading_pair}｜{contract_type}｜{margin_mode}{leverage_display}\n"
                   f"交易訂單：{order_id}\n"
                   ...
                   f"強平價格(USDT)：{liquidation_price_display}{pnl_display}{tp_sl_display}")
        else:
        ...
        if success:
            ...
            logger.info(f"BTC交易記錄保存結果: {frontend_result}")
        else:
            # 提交失敗時，記錄交易詳情和失敗原因到文件
        ...
        if positions:
            for pos in positions:
                if pos.get('symbol') == symbol:
                    ...
                    # 計算持倉數量(USDT) = 持倉大小 * 入場價格
                    position_amt = float(pos.get('positionAmt', 0))
                    entry_price = float(pos.get('entryPrice', 0))
                    position_usdt = abs(position_amt) * entry_price if entry_price > 0 else 0
...
def safe_order_id(order_id):
    """確保order_id為字符串類型，防止encode錯誤 - 金融級數據類型安全"""
    if order_id is None:
        return '--'
    return str(order_id)

def safe_hash_string(value):
    """安全地為任何值生成hash字符串，確保類型安全"""
    if value is None:
        return 'none'
    return str(value)

def convert_order_type_to_display(order_type):
    """統一轉換訂單類型格式為中文顯示格式（簡化版：只區分市價/限價）"""
    if not order_type:
        return '市價單'
    ...
    # 🔧 簡化邏輯：只區分市價執行 vs 限價執行
    
    # 所有市價執行的訂單類型（包括止盈止損市價單）
    market_execution_types = [
        'MARKET', 'MKT', '市價單', '市價', 'IOC', 'FOK',
        'TAKE_PROFIT_MARKET', 'STOP_MARKET', 'TP_MARKET', 'SL_MARKET'
    ]
    
    # 所有限價執行的訂單類型（包括止盈止損限價單）
    limit_execution_types = [
        'LIMIT', 'LMT', '限價單', '限價', 'ROD', 'GTD', 'GTC',
        'TAKE_PROFIT', 'STOP', 'TP', 'SL', 'TAKE_PROFIT_LIMIT', 'STOP_LIMIT'
    ]
    
    if order_type_str in market_execution_types:
        return '市價單'
    elif order_type_str in limit_execution_types:
        return '限價單'
    else:
        # 🔧 智能兜底判斷：根據關鍵詞判斷執行方式
        if 'MARKET' in order_type_str or 'MKT' in order_type_str:
            return '市價單'
        else:
            return '限價單'
...
def send_btc_order_modify_notification(order_data, old_data, new_data):
    ...
    try:
        ...
        action_type, direction = get_btc_action_and_direction(trade_record)
        direction = f"{action_type}{direction}"  # 組合成 "開倉多單" 或 "平倉空單" 格式
        order_type_display = convert_order_type_to_display(order_type)
...
def get_btc_action_and_direction(trade_record):
    ...
    if is_close_action:
        ...
        logger.info(f"✅ 平倉判斷: SELL平多單, BUY平空單 -> {side}平{direction_display}")
    else:
        # 開倉邏輯
        action_type = "開倉"
        if side == 'BUY':
        ...
        logger.info(f"✅ 開倉判斷: BUY開多單, SELL開空單 -> {side}開{direction_display}")
    
    logger.info(f"🎯 BTC方向判斷最終結果: action_type={action_type}, direction_display={direction_display}")
...
def send_btc_trade_notification(trade_record):
    ...
    try:
        ...
        fill_price = float(trade_record.get('fill_price', trade_record.get('price', 0)))
        ...
        if reduce_only and trade_record.get('pre_close_data'):
            ...
            position_data = {
                ...
                'margin_mode': '全倉', 
                'leverage': '100',
                'position_usdt': pre_data['position_value'],  # 這裡使用正確的字段
                'entry_price': pre_data['entry_price'],
                'liquidation_price': pre_data['liquidation_price']
            }
            logger.info(f"✅ 成交通知獲取到預保存數據: 持倉價值${pre_data['position_value']:,.2f}, 開倉價${pre_data['entry_price']:,.2f}")
        else:
            # 開倉：獲取當前數據
            position_data = get_btc_position_data_for_notification(symbol)
            if not reduce_only:
                logger.info(f"📊 成交通知使用當前數據: 開倉操作，訂單{order_id}")
            else:
                logger.warning(f"⚠️ 成交通知：平倉操作但沒有預保存數據! reduce_only={reduce_only}, 訂單{order_id}")
                logger.warning(f"⚠️ trade_record keys: {list(trade_record.keys())}")
        
        trading_pair = position_data.get('symbol', 'BTCUSDT')
        contract_type = position_data.get('contract_type', '永續')
        margin_mode = position_data.get('margin_mode', '全倉')
        leverage = position_data.get('leverage', '100')
        ...
        is_close_action = (
            reduce_only or
            'cover' in action_type.lower() or
            '平倉' in action_type or
            'close' in action_type.lower()
        )
        
        if is_close_action:
            # 平倉：顯示平倉前的持倉數據
            # 🔥 修復：優先使用預保存數據計算正確的持倉數量
            if reduce_only and trade_record.get('pre_close_data'):
                pre_data = trade_record['pre_close_data']
                # 使用平倉前的實際持倉數據計算持倉數量
                position_usdt = pre_data.get('position_value', 0)
                entry_price = pre_data.get('entry_price', 0)
                liquidation_price = pre_data.get('liquidation_price', 0)
                logger.info(f"🔧 成交通知使用平倉前數據: 持倉USDT={position_usdt:.2f}, 開倉價格={entry_price:.2f}")
            else:
                # 回退邏輯：使用position_data
                position_usdt = position_data.get('position_usdt', position_data.get('position_value', 0))
                entry_price = position_data.get('entry_price', 0)
                liquidation_price = position_data.get('liquidation_price', 0)
                logger.warning(f"⚠️ 成交通知無平倉前數據，使用回退邏輯: position_usdt={position_usdt}")
            
            # 平倉價格使用成交價格
            price_label = "平倉價格(USDT)"
            price_display = f"{fill_price:,.2f}" if fill_price > 0 else "市價"
            
            # 強平價格顯示（平倉後通常顯示N/A）
            liquidation_price_display = "已平倉"
            
        else:
            # 開倉：使用當前數據
            position_usdt = position_data.get('position_usdt', btc_quantity * fill_price)
...
def send_btc_trading_statistics():
    ...
    try:
        ...
        # 1. 獲取今日交易統計（從BTC數據文件計算真實數量）
        total_orders = get_btc_message_count_today('提交成功')  # 委託次數：提交成功記錄數
        total_cancels = get_btc_message_count_today('提交失敗')    # 取消次數：提交失敗記錄數  
        total_deals = get_btc_message_count_today('成交通知')     # 成交次數：成交通知記錄數
        ...
        if binance_client:
            try:
                ...
                # 獲取盈虧數據 - 使用與前端一致的幣安公式（已實現+未實現-資金費用）
                ...
                account_data['todayPnl'] = today_pnl
                account_data['week7Pnl'] = week_pnl
                account_data['month30Pnl'] = month_pnl
        ...
        # 8. 記錄後端日誌（文件記錄已在send_btc_telegram_message中記錄）
...
def btc_webhook():
    ...
    try:
        ...
        if hasattr(g, 'webhook_processed') and g.webhook_processed:
            logger.warning("🚨 BTC Webhook重複處理檢測：Flask g.webhook_processed已設置")
            return jsonify({'success': True, 'message': 'Webhook請求已被處理，跳過重複處理'})
        
        # 全局請求去重檢查
        global_key = f"btc_webhook_{request_hash}"
        if not hasattr(btc_webhook, 'processed_requests'):
            btc_webhook.processed_requests = {}
        
        # 清理超過30秒的舊請求
        btc_webhook.processed_requests = {
            k: v for k, v in btc_webhook.processed_requests.items() 
            if current_time - v < 30
        }
        
        if global_key in btc_webhook.processed_requests:
            time_diff = current_time - btc_webhook.processed_requests[global_key]
            logger.warning(f"🚨 BTC Webhook重複處理檢測：相同請求在{time_diff:.1f}秒前已處理")
            return jsonify({'success': True, 'message': f'相同請求已在{time_diff:.1f}秒前處理'})
        
        # 標記請求為已處理
        btc_webhook.processed_requests[global_key] = current_time
        g.webhook_processed = True
        
        logger.info(f"🔍 BTC Webhook新請求：{request_hash[:8]}...")
        ...
        # 🔧 新增：調試日誌 - 記錄原始訊號數據
        logger.info(f"📥 BTC訊號調試 - 原始數據: {data}")
        ...
        # 🔧 重構訊號解析邏輯：分層匹配，避免關鍵詞衝突
        # 簡化關鍵詞匹配，只保留必要的4個關鍵詞
        close_patterns = {
            '平多': 'CLOSE_LONG',  # 平多 = 僅平多單
            '平空': 'CLOSE_SHORT'  # 平空 = 僅平空單
        }
        ...
        if action == '平多':
            action = 'CLOSE_LONG'
            matched_pattern = "action字段: 平多"
            action_found = True
            logger.info(f"🔒 平多訊號 -> 僅平多單")
        elif action == '平空':
            action = 'CLOSE_SHORT'
            matched_pattern = "action字段: 平空"
            action_found = True
            logger.info(f"🔒 平空訊號 -> 僅平空單")
        elif action == '開多':
            action = 'LONG'
            matched_pattern = "action字段: 開多"
            action_found = True
            logger.info(f"🔓 開多訊號")
        elif action == '開空':
            action = 'SHORT'
            matched_pattern = "action字段: 開空"
            action_found = True
            logger.info(f"🔓 開空訊號")
        
        # 第一層：檢查直接匹配（最快路徑）
        if not action_found:
            all_patterns = {**close_patterns, **open_patterns}
            if action in all_patterns:
                action = all_patterns[action]
                matched_pattern = f"直接匹配: {action}"
                action_found = True
                logger.info(f"🎯 {matched_pattern}")
            ...
            for group_name, patterns in pattern_groups:
                ...
                for key, value in sorted_patterns:
                    if key in raw_message:
                        logger.info(f"🎯 {group_name}匹配: '{key}' -> '{value}' (訊息: '{raw_message}')")
                        action = value
                        matched_pattern = f"{group_name}: {key}"
                        action_found = True
                        break
        ...
        if action in ['CLOSE', 'CLOSE_LONG', 'CLOSE_SHORT']:
            ...
            try:
                if binance_client:
                    ...
                    has_position = False
                    for pos in positions:
                        if pos.get('symbol') == 'BTCUSDT' and float(pos.get('positionAmt', 0)) != 0:
                            has_position = True
                            break
                    
                    if not has_position:
                        logger.warning(f"🚫 收到平倉訊號但無持倉，直接跳過避免重複下單")
                        return jsonify({'success': True, 'message': '無持倉可平倉，跳過處理'})
            except Exception as e:
                logger.error(f"檢查持倉狀態失敗: {e}")
            
            # 強化去重：基於動作類型而非具體訊號內容
            import hashlib
            # 使用更簡單的去重key，減少因為細微差異導致去重失效
            simple_action_key = f"BTC-{action}"
            
            current_time = datetime.now()
            signal_key = f"{simple_action_key}-{current_time.strftime('%Y%m%d%H%M%S')}"  # 精確到秒
            
            with btc_signals_lock:
                logger.info(f"🔍 檢查平倉訊號去重: {action}, 當前時間: {current_time.strftime('%H:%M:%S')}")
                
                # 檢查精確時間重複
                if signal_key in btc_processed_signals:
                    logger.warning(f"🚫 BTC平倉重複訊號已跳過(精確時間): {signal_key}")
                    return jsonify({'success': True, 'message': '平倉重複訊號已跳過'})
                
                # 🔧 強化：檢查30秒內相同平倉動作的重複（延長保護時間）
                duplicate_found = False
                for existing_signal in btc_processed_signals:
                    if existing_signal.startswith(f"{simple_action_key}-"):
                        existing_time_str = existing_signal.split('-')[-1]
                        try:
                            existing_time = datetime.strptime(existing_time_str, '%Y%m%d%H%M%S')
                            time_diff = (current_time - existing_time).total_seconds()
                            if time_diff < 30:  # 延長到30秒保護
                                logger.warning(f"🚫 BTC 30秒內相同平倉動作重複訊號已跳過: {action}, 時間差: {time_diff:.1f}秒")
                ...
                if duplicate_found:
                    return jsonify({'success': True, 'message': '30秒內相同平倉動作重複訊號已跳過'})
                
                # 記錄平倉訊號
                btc_processed_signals.add(signal_key)
                logger.info(f"✅ 平倉訊號已記錄: {signal_key}")
                
                # 清理超過90秒的舊訊號記錄（延長清理時間）
                cutoff_time = (current_time - timedelta(seconds=90)).strftime('%Y%m%d%H%M%S')
                old_count = len(btc_processed_signals)
                btc_processed_signals = {s for s in btc_processed_signals if s.split('-')[-1] >= cutoff_time}
                new_count = len(btc_processed_signals)
                if old_count != new_count:
                    logger.info(f"🧹 清理舊訊號記錄: {old_count} -> {new_count}")
        ...
        if action in ['LONG', 'SHORT']:
            # 進場信號 - 不進行去重檢查，直接執行（保持原有邏輯）
            logger.info(f"🔓 BTC開倉訊號無去重限制: {action}")
            order_result = process_btc_unified_signal(data, action)
        elif action in ['CLOSE_LONG', 'CLOSE_SHORT']:
            # 平倉訊號：只平倉對應方向持倉，不開倉
            logger.info(f"🔒 BTC平倉訊號: {action} -> 僅平倉不開倉")
            
            # 直接調用平倉處理函數
            order_result = handle_pure_close_signal('BTCUSDT', action)
            logger.info(f"🔍 平倉處理完成，結果: {order_result}")
            
            # 🔧 重要檢查：確保平倉後沒有觸發其他操作
            if order_result is None:
                logger.info("✅ 平倉訊號處理完成：無持倉需要平倉")
                return jsonify({'success': True, 'message': '無持倉需要平倉，訊號處理完成'})
            elif isinstance(order_result, dict) and order_result.get('warning'):
                logger.info(f"✅ 平倉訊號處理完成：{order_result['warning']}")
                return jsonify({'success': True, 'message': order_result['warning']})
            else:
                logger.info("✅ 平倉訊號處理完成：平倉訂單已提交")
                # 🔧 關鍵：平倉完成後立即返回，避免執行後續邏輯
                return jsonify({
                    'success': True, 
                    'message': 'BTC平倉訊號處理成功',
                    'action': action,
    ...
    try:
        ...
        if binance_client and api_status == "已連線":
            try:
                ...
                account_data['todayPnl'] = today_pnl
                account_data['todayPnlPercent'] = today_pnl_percent
                account_data['week7Pnl'] = week_pnl
                account_data['week7PnlPercent'] = week_pnl_percent
                account_data['month30Pnl'] = month_pnl
                account_data['month30PnlPercent'] = month_pnl_percent
        ...
        if telegram_success:
            # 文件記錄已在send_btc_telegram_message中記錄
            logger.info(f"BTC啟動通知已發送")
            return True
        else:
            # 失敗時需要額外記錄，因為send_btc_telegram_message返回False時不會記錄文件
...
def get_btc_message_count_today(message_type):
    ...
    try:
        ...
        if not os.path.exists(filename):
            logger.info(f"BTC交易數據文件不存在: {filename}")
            return 0
        ...
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                ...
                if not isinstance(trades_data, list):
                    logger.warning(f"BTC交易數據文件格式異常: {filename}")
                    return 0
        except Exception as e:
            logger.error(f"讀取BTC交易數據文件失敗: {e}")
            return 0
        
        # 統計特定類型的記錄數量（參考TX系統邏輯）
        count = 0
        for record in trades_data:
            ...
            # 根據message_type統計對應的記錄
            if message_type == '提交成功' and record_type == 'order':
                count += 1
            elif message_type == '成交通知' and record_type == 'deal':
                count += 1
            elif message_type == '提交失敗' and record_type == 'fail':
                count += 1
            elif message_type in message:  # 降級匹配：直接搜索消息內容
                count += 1
        
        logger.info(f"BTC統計({message_type}): {count}筆")
        return count
...
def cleanup_old_btc_data_files():
    """清理舊的BTC交易記錄文件，保留30天（參考TX系統）"""
    try:
        from trading_config import TradingConfig
        import os
        ...
        # 獲取所有BTC交易記錄文件
        btc_files = []
        for filename in os.listdir(btc_data_dir):
            if filename.startswith('BTCtransdata_') and filename.endswith('.json'):
                try:
                    # 從文件名提取日期
                    date_str = filename.replace('BTCtransdata_', '').replace('.json', '')
                    file_date = datetime.strptime(date_str, '%Y%m%d')
                    btc_files.append((filename, file_date))
                except ValueError:
                    logger.warning(f"跳過無效的BTC文件名: {filename}")
        
        # 按日期排序（最新的在前）
        btc_files.sort(key=lambda x: x[1], reverse=True)
        
        # 保留最新30個文件，刪除其餘的
        if len(btc_files) > 30:
            files_to_delete = btc_files[30:]
            for filename, file_date in files_to_delete:
                file_path = os.path.join(btc_data_dir, filename)
                try:
                    os.remove(file_path)
                    logger.info(f"已刪除舊的BTC交易記錄: {filename}")
                except Exception as e:
                    logger.error(f"刪除BTC文件失敗 {filename}: {e}")
                    
        logger.info(f"BTC文件清理完成，保留 {min(len(btc_files), 30)} 個文件")
        
    except Exception as e:
        logger.error(f"BTC文件清理失敗: {e}")

def test_btc_file_persistence():
    """測試BTC文件持久化機制"""
    try:
        logger.info("🧪 測試BTC文件持久化機制...")
        
        # 測試日誌記錄
        log_btc_frontend_message("Telegram［提交成功］訊息發送成功！！！", "success")
        log_btc_frontend_message("Telegram［成交通知］訊息發送成功！！！", "success")
        log_btc_frontend_message("Telegram［提交失敗］訊息發送成功！！！", "error")
        
        # 測試統計查詢
        order_count = get_btc_message_count_today('提交成功')
        deal_count = get_btc_message_count_today('成交通知')
        fail_count = get_btc_message_count_today('提交失敗')
        
        logger.info(f"✅ BTC文件持久化測試完成:")
        logger.info(f"   委託次數: {order_count}")
        logger.info(f"   成交次數: {deal_count}")
        logger.info(f"   失敗次數: {fail_count}")
        
        return True
        
    except Exception as e:
        logger.error(f"BTC文件持久化測試失敗: {e}")
        return False

def diagnose_btc_data_system():
    """診斷BTC數據持久化系統狀態（參考TX系統）"""
    try:
        from trading_config import TradingConfig
        import os
        import json
        ...
        logger.info("🔍 開始執行BTC數據系統診斷...")
        
        btc_data_dir = TradingConfig.BTC_DATA_DIR
        today = datetime.now().strftime("%Y%m%d")
        today_file = os.path.join(btc_data_dir, f"BTCtransdata_{today}.json")
        
        diagnosis_result = {
            'system_info': {
                'btc_data_dir': btc_data_dir,
                'directory_exists': os.path.exists(btc_data_dir),
        ...
        # 檢查目錄狀態
        if not os.path.exists(btc_data_dir):
            logger.error(f"❌ BTC數據目錄不存在: {btc_data_dir}")
            diagnosis_result['system_info']['status'] = 'directory_missing'
        ...
        if os.path.exists(today_file):
            try:
                with open(today_file, 'r', encoding='utf-8') as f:
                    ...
                    diagnosis_result['statistics']['today_records'] = len(today_data)
                    
                    # 統計記錄類型
                    for record in today_data:
                        record_type = record.get('type', '')
                        if record_type == 'order':
                            diagnosis_result['statistics']['order_count'] += 1
                        elif record_type == 'deal':
                            diagnosis_result['statistics']['deal_count'] += 1
                        elif record_type == 'fail':
                            diagnosis_result['statistics']['fail_count'] += 1
                            
                logger.info(f"✅ 今日BTC數據文件正常: {diagnosis_result['statistics']['today_records']}筆記錄")
            except Exception as e:
                logger.error(f"❌ 今日BTC數據文件讀取失敗: {e}")
                diagnosis_result['file_analysis']['today_file_error'] = str(e)
        
        # 統計所有BTC文件
        try:
            all_files = [f for f in os.listdir(btc_data_dir) if f.startswith('BTCtransdata_') and f.endswith('.json')]
            diagnosis_result['statistics']['total_files'] = len(all_files)
            logger.info(f"📊 BTC數據文件總數: {len(all_files)}個")
        except Exception as e:
            logger.error(f"❌ 統計BTC文件失敗: {e}")
        
        logger.info("✅ BTC數據系統診斷完成")
        return diagnosis_result
        
    except Exception as e:
        logger.error(f"BTC數據系統診斷失敗: {e}")
        return {'error': str(e)}
...
def get_btc_trading_statistics_data(date_str=None):
    ...
    try:
        ...
        # 1. 從BTC數據文件統計委託、取消、成交次數
        order_count = get_btc_message_count_today('提交成功')  # 委託次數（提交成功記錄數）
        cancel_count = get_btc_message_count_today('提交失敗')  # 取消次數（提交失敗記錄數）
        fill_count = get_btc_message_count_today('成交通知')   # 成交次數（成交通知記錄數）
    ...
    try:
        ...
        if is_monthly and monthly_data:
            ...
            try:
                ...
                # 7天和30天盈虧 - 使用與前端一致的幣安公式
                week_pnl, week_pnl_percent = get_period_total_pnl_binance_formula(7)
                month_pnl, month_pnl_percent = get_period_total_pnl_binance_formula(30)
                account['week_pnl'] = week_pnl
                account['month_pnl'] = month_pnl
...
def get_btc_position_from_json(is_long, position_size, entry_price):
    ...
    try:
        ...
        price_tolerance = entry_price * 0.02   # 2%的價格容差（放寬）
        quantity_tolerance = position_size * 0.2  # 20%的數量容差（放寬）
                        ...
                        if action_match and status_match and price_match and quantity_match:
                            ...
                            if timestamp:
                                try:
                                    # 🔧 改進時間解析邏輯
                                    if 'T' in timestamp:
                                        # ISO格式: "2025-08-21T20:00:46.490758"
                                        timestamp_clean = timestamp.replace('Z', '').split('.')[0]  # 移除毫秒
                                        open_time = datetime.fromisoformat(timestamp_clean)
                                    else:
                                        open_time = datetime.fromisoformat(timestamp)
                                    open_time_str = open_time.strftime('%Y/%m/%d %H:%M:%S')
                                except Exception as time_error:
                                    logger.warning(f"時間解析失敗 '{timestamp}': {time_error}")
                                    # 備用解析方法
                                    try:
                                        open_time = datetime.strptime(timestamp[:19], '%Y-%m-%dT%H:%M:%S')
                                        open_time_str = open_time.strftime('%Y/%m/%d %H:%M:%S')
                                    except:
                                        open_time_str = timestamp[:19] if len(timestamp) >= 19 else 'N/A'
                            else:
                                open_time_str = 'N/A'
                            
                            # 🔧 改進訂單ID獲取邏輯
                            order_id = str(open_record.get('order_id', open_record.get('deal_order_id', 'N/A')))
                            
                            # 🔧 改進來源判斷邏輯
                            source = open_record.get('source', '')
                            if not source or source == '':
                                source = 'manual'  # 空字符串默認為手動
    ...
    try:
        ...
        if open_positions:
            for pos in open_positions:
                values4 = [
                    pos.get('open_time', ''),   # 開倉時間 (修正字段名)
                    pos.get('order_id', ''),    # 交易單號 (修正字段名)
...
def get_detailed_trade_records(date_str):
    ...
    try:
        ...
        # 獲取當前持倉
        positions = binance_client.futures_position_information() if binance_client else []
            ...
            for pos in positions:
                ...
                if position_amt != 0:
                    open_positions.append({
                        'open_time': '',   # 開倉時間，需要從交易歷史獲取 (修正字段名)
                        'order_id': '',    # 交易單號 (修正字段名)
                        'symbol': pos.get('symbol', 'BTCUSDT'),
                        'action': '開倉',  # 交易動作
                        'order_type': '市價',  # 交易類別
    ...
    try:
        ...
        try:
            ...
            current_account = {
                ...
                'unrealized_pnl': pnl,
                'margin_ratio': margin_ratio,
                'today_commission': total_stats['commission']  # 使用月報累計手續費
            }
        except Exception as e:
            ...
            current_account = {
                ...
                'unrealized_pnl': 0,
                'margin_ratio': 0,
                'today_commission': total_stats['commission']  # 即使異常也使用累計手續費
            }
...
# 🔧 此函數已移除：重複的動作判斷邏輯已統一使用 get_btc_action_and_direction()
...
def process_order_fill(order_id, fill_price, fill_quantity):
    ...
    try:
        ...
        if direction == '多':
            ...
            # 備用邏輯：根據action判斷
            record_side = 'BUY' if direction == '多' else 'SELL'
        ...
        fill_trade_record = {
            ...
            'quantity': float(fill_quantity),
            'price': float(fill_price),
            'order_id': str(order_id),  # 🔧 確保order_id為字符串類型
            'order_type': order_type,
            'source': source,
            'action_type': action,
        ...
        # 🔧 如果是平倉操作，嘗試獲取並添加平倉前數據
        if 'cover' in action.lower() and str(order_id) in btc_active_trades:
            active_trade = btc_active_trades[str(order_id)]
            pre_close_data = active_trade.get('pre_close_data')
            if pre_close_data:
                fill_trade_record['pre_close_data'] = pre_close_data
                logger.info(f"✅ 成交通知已添加平倉前數據: 持倉價值{pre_close_data.get('position_value', 0):.2f}USDT")
        ...
        logger.info(f"準備將BTC成交通知加入排隊: 訂單{order_id}")
        btc_notification_manager.add_notification('fill', fill_trade_record, True, fill_delay)
        logger.info(f"BTC成交通知已加入排隊系統（延遲{fill_delay:.1f}秒）")
        ...
        def delayed_frontend_log():
            try:
                ...
                logger.info(f"系統訂單交易記錄已保存: {order_id}")
            except Exception as e:
                logger.error(f"延遲記錄交易失敗: {e}")
...
# 已移除btc_place_order函數 - 使用process_btc_unified_signal統一處理交易
def get_btc_config():
    """獲取BTC配置"""
    try:
        env_data = load_btc_env_data()
                    ...
                    if data.get('e') == 'ORDER_TRADE_UPDATE':
                        ...
                        if order_id in btc_order_history:
                            ...
                            if (price_changed or quantity_changed) and status not in ['FILLED', 'CANCELED', 'REJECTED', 'EXPIRED']:
                                ...
                                # 訂單修改通知已禁用（用戶要求）
                                logger.info(f"🔕 訂單修改檢測到但通知已禁用: {order_id}, 價格變更: {price_changed}, 數量變更: {quantity_changed}")
                                # send_btc_order_modify_notification(order_data, old_data, new_data) # 已禁用
                        ...
                        if status == 'NEW':
                            # 新訂單提交 - 檢查是否已處理，避免重複通知
                            processed_key = f"{order_id}_NEW"
                            with processed_orders_lock:
                                already_processed = processed_key in processed_orders
                                if not already_processed:
                                    # 立即標記為已處理，避免並發重複
                                    processed_orders.add(processed_key)
                            
                            if already_processed:
                                logger.info(f"🔄 WebSocket跳過已處理的NEW訂單: {order_id}")
                            elif symbol == 'BTCUSDT':
                            ...
                            # 訂單完全成交 - 檢查是否已處理，避免重複通知  
                            filled_key = f"{order_id}_FILLED"
                            with processed_orders_lock:
                                already_filled = filled_key in processed_orders
                                if not already_filled:
                                    # 立即標記為已處理，避免並發重複
                                    processed_orders.add(filled_key)
                            
                            if already_filled:
                                logger.info(f"🔄 WebSocket跳過已處理的FILLED訂單: {order_id}")
                            elif symbol == 'BTCUSDT':
                            ...
                            # 檢查是否已處理過此失效狀態，避免重複通知
                            failed_key = f"{order_id}_{status}"
                            with processed_orders_lock:
                                already_failed = failed_key in processed_orders
                                if not already_failed:
                                    # 立即標記為已處理，避免並發重複
                                    processed_orders.add(failed_key)
                            
                            if already_failed:
                                logger.info(f"🔄 WebSocket跳過已處理的{status}訂單: {order_id}")
                            else:
                                ...
                                try:
                                    ...
                                    # 對於錯誤通知，使用基於錯誤時間的延遲計算
                                    error_id = f"error_{int(time.time() * 1000)}"
                                    error_hash = int(hashlib.md5(safe_hash_string(error_id).encode()).hexdigest()[:8], 16)
                                    error_base_delay = (error_hash % 100) * 0.1
                                    error_submit_delay = error_base_delay + 1
                                    
                                    btc_notification_manager.add_notification('submit', error_record, False, error_submit_delay)
                                    ...
                                    # 🔧 修復：只清理非失效狀態的記錄，保持失效狀態的重複保護
                                    orders_to_remove = []
                                    failed_statuses = ['_CANCELED', '_REJECTED', '_EXPIRED']
                                    for processed_key in processed_orders:
                                        if (processed_key.startswith(f"{order_id}_") and 
                                            not any(processed_key.endswith(failed_status) for failed_status in failed_statuses)):
                                    ...
                                    # 🔧 保持失效狀態記錄，防止重複通知
                                    logger.info(f"🔄 訂單 {order_id} 已清理(保留失效狀態記錄防重複)，允許重新提交")
...
def handle_manual_binance_order(order_data, is_success=True):
    ...
    try:
        ...
        trade_record = {
            ...
            'reduceOnly': reduce_only,
            'reduce_only': reduce_only,
            'action_type': ''  # WebSocket數據沒有action_type，由函數自動判斷
        }
        parsed_action, direction = get_btc_action_and_direction(trade_record)
        ...
        if order_type == 'MARKET':
            ...
            price_display = "市價"  # 止盈止損市價單
        else:
            price_display = f"{float(order_data.get('p', 0)):,.2f}"
        ...
        # 🔧 如果是手動平倉提交，獲取當前持倉數據用於通知
        current_position_data = None
        if reduce_only and is_manual:
            logger.info(f"🔍 手動平倉提交{order_id}，獲取當前持倉數據...")
            try:
                position_amt, entry_price = get_btc_actual_position_info(symbol)
                if position_amt != 0 and entry_price > 0:
                    ...
                    logger.info(f"✅ 手動平倉獲取持倉數據: {abs(position_amt):.8f} BTC, 開倉價格: ${entry_price:.2f}")
                else:
                    logger.warning(f"⚠️ 未獲取到有效持倉數據: 數量={position_amt}, 價格={entry_price}")
            except Exception as e:
                logger.error(f"❌ 獲取手動平倉持倉數據失敗: {e}")
        ...
        if current_position_data:
            trade_record['pre_close_data'] = current_position_data
        ...
        # 🔧 使用基於訂單ID的延遲，確保同訂單通知成組發送
        # 計算該訂單的基礎延遲時間 (基於訂單ID hash確保唯一性)
        import hashlib
        order_hash = int(hashlib.md5(safe_hash_string(order_id).encode()).hexdigest()[:8], 16)
        base_delay = (order_hash % 100) * 0.1  # 0-10秒的基礎延遲，避免通知衝突
        
        submit_delay = base_delay + 1  # 提交通知：基礎延遲 + 1秒
        logger.info(f"準備將{order_source_display}幣安訂單提交通知加入排隊: {trade_record}")
        btc_notification_manager.add_notification('submit', trade_record, is_success, submit_delay)
        logger.info(f"{order_source_display}幣安訂單提交通知已加入排隊系統（延遲{submit_delay:.1f}秒）")
...
def handle_manual_binance_fill(order_data):
    ...
    try:
        ...
        action, direction = get_btc_action_and_direction(trade_record)
        ...
        # 🔧 檢查是否有預保存的平倉前數據，手動平倉時實時獲取
        pre_close_data = None
        if reduce_only:
            if order_id in btc_active_trades:
                # 從活躍交易記錄中獲取預保存數據（自動對翻情況）
                active_trade = btc_active_trades[order_id]
                pre_close_data = active_trade.get('pre_close_data')
                if pre_close_data:
                    logger.info(f"✅ 成交通知獲取到預保存數據: 訂單{order_id}, 平倉前持倉${pre_close_data['position_value']:,.2f}")
            else:
                # 手動平倉：實時獲取持倉數據
                logger.info(f"🔍 手動平倉訂單{order_id}，實時獲取持倉數據...")
                try:
                    actual_position_amt, actual_entry_price = get_btc_actual_position_info(symbol)
                    
                    # 由於是平倉成交後，持倉已經變化，我們嘗試從配對記錄中獲取原始持倉信息
                    if actual_position_amt == 0:
                        # 平倉後持倉為0，嘗試從BTC JSON記錄中獲取原始持倉
                        logger.info("🔍 平倉後持倉為0，嘗試從交易記錄推算原始持倉...")
                        # 計算原始持倉：成交數量就是被平掉的持倉數量
                        original_position_amt = float(quantity)
                        
                        # 獲取當前價格作為參考
                        try:
                            ticker = binance_client.get_ticker_price(symbol=symbol)
                            current_price = float(ticker['price']) if ticker else avg_price
                        except:
                            current_price = avg_price
                            
                        pre_close_data = {
                            'position_amt': original_position_amt,
                            'entry_price': avg_price,  # 使用成交價作為參考
                            'position_value': original_position_amt * current_price
                        }
                        logger.info(f"✅ 手動平倉推算持倉數據: {original_position_amt:.8f} BTC, 參考價格: ${current_price:.2f}")
                    else:
                        # 部分平倉情況，計算剩餘持倉
                        original_position_amt = actual_position_amt + float(quantity)
                        position_value = original_position_amt * actual_entry_price if actual_entry_price > 0 else 0
                        ...
                        logger.info(f"✅ 手動部分平倉持倉數據: 原持倉{original_position_amt:.8f} BTC, 開倉價格: ${actual_entry_price:.2f}")
                        
                except Exception as e:
                    logger.error(f"❌ 實時獲取持倉數據失敗: {e}")
                    # 備用方案：使用成交數量作為持倉數量
                    pre_close_data = {
                        'position_amt': float(quantity),
                        'entry_price': avg_price,
                        'position_value': float(quantity) * avg_price
                    }
                    logger.info(f"🔧 使用備用方案: 持倉數量={quantity} BTC, 參考價格=${avg_price:.2f}")
        ...
        fill_trade_record = {
            ...
            'side': side,
            'quantity': float(quantity),
            'fill_price': float(avg_price),  # 成交價格字段
            'price': float(avg_price),      # 保持兼容性
            'order_id': order_id,
            'order_type': order_type_text,
            'source': 'manual' if order_source == '手動' else 'webhook',
            'action_type': action,
        ...
        # 🔧 成交通知使用相同訂單的基礎延遲 + 3秒，確保在提交通知後發送
        import hashlib
        order_hash = int(hashlib.md5(safe_hash_string(order_id).encode()).hexdigest()[:8], 16)
        base_delay = (order_hash % 100) * 0.1  # 與提交通知使用相同的基礎延遲
        fill_delay = base_delay + 3  # 成交通知：基礎延遲 + 3秒 (在提交通知後2秒)
        
        logger.info(f"準備將{order_source}BTC成交通知加入排隊: 訂單{order_id}")
        btc_notification_manager.add_notification('fill', fill_trade_record, True, fill_delay)
        logger.info(f"{order_source}BTC成交通知已加入排隊系統（延遲{fill_delay:.1f}秒）")
        ...
        def delayed_frontend_log():
            try:
                ...
                log_btc_frontend_message(fill_log, "success")
                logger.info(f"{order_source}幣安訂單交易記錄已保存: {order_id}")
            except Exception as e:
                logger.error(f"延遲記錄{order_source}交易失敗: {e}")
...
def get_btc_trading_config():
    ...
    try:
        ...
        # 2. 實際API設置
        try:
            position_risk = binance_client.futures_position_information(symbol='BTCUSDT')
