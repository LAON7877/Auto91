...
// 全局變數初始化
window.currentTunnelType = 'tx'; // 默認為 TX
...
const SystemLogManager = {
    ...
    async updateSystemLogs(systemType = 'tx') {
        ...
        const globalLogsArray = 'systemLogs';
        
        try {
            const result = await APIUtils.get(apiEndpoint);
            if (result.status === 'success' && result.logs) {
                const filterCondition = (log => {
    ...
    updateSystemLogsDisplay(systemType = 'tx') {
        const containerId = 'system-logs-content';
        const logsArray = window.systemLogs || [];
...
function saveEnv(e) {
    ...
    .then(data => {
        ...
        // 更新永豐API狀態
        // 移除重複調用，使用專用的updateSinopacApiStatus函數
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 域名模式將由 loadTunnelSettings() 設定，不使用硬編碼預設值
    ...
    setInterval(updateTradingStatus, 30000); // 每30秒更新交易狀態（確保開市/休市狀態及時更新）
...
function startAccountAutoUpdate() {
    ...
    // 每1分鐘檢查一次
    window.accountUpdateInterval = setInterval(updateAccountData, 60000);
...
// 啟動BTC帳戶自動更新 - BTC 24/7市場，每1分鐘定時更新
...
function showTunnelSetup() {
    ...
    // 載入已儲存的隧道設定
    loadTunnelSettingsToModal();
}

// 載入隧道設定到模態窗口
function loadTunnelSettingsToModal() {
    fetch('/api/tunnel/settings')
        .then(response => response.json())
        .then(data => {
            if (data.success && data.settings) {
                const mode = data.settings.domain_mode || 'temporary';
                
                // 設置域名模式選項
                setDomainMode(mode, null, false);
                
                // 如果是自訂模式且有數據，填入表單
                if (mode === 'custom' && data.settings.has_saved_data) {
                    // 等待一下確保DOM元素已創建
                    setTimeout(() => {
    ...
    try {
        fetch('/api/tunnel/settings')
            ...
            .catch(error => {
                ...
                setDomainModeForBothSides(selectedDomainMode); // 確保UI也設為預設值
            });
    } catch (error) {
        selectedDomainMode = 'temporary'; // fallback to default
        setDomainModeForBothSides(selectedDomainMode); // 確保UI也設為預設值
